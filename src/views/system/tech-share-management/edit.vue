...
defineOptions({ name: "TechShareEdit" });
...
const filteredCategoryOptions = computed(() => {
  return categoryOptions.value.filter(cat => cat.name === "技术分享");
});
...
const getDraftKey = () => `tech_share_draft_${articleId.value || "new"}`;
const initPage = async () => {
  ...
  try {
    const fetchOptionsPromise = Promise.all([
      ...
      // 如果是新建，自动设置"技术分享"分类
      if (id === "new") {
        const techShareCategory = catRes.data.find(cat => cat.name === "技术分享");
        if (techShareCategory) {
          form.post_category_ids = [techShareCategory.id];
        }
      }
    });
    if (id !== "new") {
      ...
      // 如果编辑时没有"技术分享"分类，自动添加
      const techShareCategory = categoryOptions.value.find(cat => cat.name === "技术分享");
      if (techShareCategory && !form.post_category_ids.includes(techShareCategory.id)) {
        form.post_category_ids.push(techShareCategory.id);
      }
...
const onSaveHandler = async (markdown: string, sanitizedHtml: string) => {
  ...
  try {
    ...
    if (isEditMode.value) {
      ...
      await router.replace({ name: "TechShareEdit", params: { id: newArticleId } });
    }
    localStorage.removeItem(getDraftKey());
    updateInitialState();
...
const handleGoBack = () => {
  if (isDirty.value) {
    ElMessageBox.confirm(
      ...
      .then(() => {
        router.push({ name: "TechShareManagement" });
      })
      .catch(() => {});
  } else {
    router.push({ name: "TechShareManagement" });
...
const handleCategoryChange = (values: string[]) => {
  // 强制固定为"技术分享"分类，不允许选择其他分类
  const techShareCategory = categoryOptions.value.find(cat => cat.name === "技术分享");
  if (techShareCategory) {
    // 如果用户尝试选择其他分类，强制重置为技术分享分类
    if (!values.includes(techShareCategory.id) || values.length > 1) {
      form.post_category_ids = [techShareCategory.id];
      categorySelectKey.value++;
      ElMessage.warning("技术分享管理的分类已固定为'技术分享'，无法选择其他分类");
    }
  }
};
...
const refreshCategories = async () => {
  try {
    ...
    // 刷新后确保"技术分享"分类存在
    const techShareCategory = data.find(cat => cat.name === "技术分享");
    if (techShareCategory && !form.post_category_ids.includes(techShareCategory.id)) {
      form.post_category_ids.push(techShareCategory.id);
    }
  } catch (error) {
