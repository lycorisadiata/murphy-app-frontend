...
// 技术分享分类ID（用于后续可能的功能扩展）
const techShareCategoryId = ref<string | null>(null);
...
// 获取技术分享分类ID
const fetchTechShareCategoryId = async () => {
  try {
    const { data } = await getCategoryList();
    const techShareCategory = data.find(cat => cat.name === "技术分享");
    if (techShareCategory) {
      techShareCategoryId.value = techShareCategory.id;
    }
  } catch (error) {
...
const fetchData = async () => {
  ...
  try {
    const params: GetArticleListParams = {
      ...
      category: "技术分享" // 默认筛选技术分享分类
    };
    const { data } = await getArticleList(params);
    // 前端再次过滤，确保只显示"技术分享"分类的文章
    const filteredList = data.list.filter(article => {
      ...
      return hasTechShareCategory && !hasProjectCategory;
    });
    state.tableData = filteredList;
    state.pagination.total = data.total;
  } catch (error) {
    ElMessage.error("获取技术分享列表失败");
  } finally {
    state.loading = false;
  }
};
...
const handleNew = () => {
  router.push({ name: "TechShareEdit", params: { id: "new" } });
};

const handleEdit = (row: Article) => {
  router.push({ name: "TechShareEdit", params: { id: row.id } });
};

const handleDelete = (row: Article) => {
  ElMessageBox.confirm(`确定要删除技术分享《${row.title}》吗？`, "提示", {
...
const handleBatchDelete = async () => {
  if (selectedArticles.value.length === 0) {
    ElMessage.warning("请先选择要删除的技术分享");
    return;
  }

  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${selectedArticles.value.length} 个技术分享吗？此操作不可恢复。`,
      "批量删除确认",
      {
    ...
    if (data.failed_count > 0) {
      ...
      ElMessage.success(`成功删除 ${data.success_count} 个技术分享`);
    }
...
onMounted(async () => {
  await fetchTechShareCategoryId();
  fetchData();
});
</script>

<template>
  <div class="tech-share-management-container">
    <!-- 顶部搜索和操作栏 -->
    ...
    <div
      v-loading="loading"
      element-loading-text="正在加载技术分享列表..."
      class="content-area"
    >
...
.tech-share-management-container {
  display: flex;
